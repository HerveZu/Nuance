...
import { playSession, user, userStats } from "@/db/schema";
import {
  getServerPuzzle,
  serverDayNumber,
  toPublicPuzzle,
...
import { offsetSchema, puzzleCompositionSchema } from "@/lib/schemas";
import type { Subject } from "@/lib/auth-guards";
...
// server actions (src/app/actions.ts) and the public HTTP API
// (src/app/api/game). The secret recipe is held here and only revealed once a
// game is finished.
 ...
 * counts as 7). Lifted out of the page so the query lives next to the rest of
 * the game's data access.
 * ------------------------------------------------------------------------- */
